...
from app.schemas.procurement import GRNCreate, GRNOut
...
def create_grn(
    ...
    # 2. Add Supplier ledger balance
    supplier.ledger_balance += Decimal(str(total_cost_calculated))

    # 3. Process each item and adjust inventory
    for item in payload.items:
        ...
        if not inv:
            # First time this variant is in this branch
            inv = Inventory(
                company_id=payload.company_id,
                branch_id=payload.branch_id,
                product_variant_id=item.product_variant_id,
            ...
            # Recalculate average cost using weighted formula
        ...
        # Update product variant cost for catalog reference
